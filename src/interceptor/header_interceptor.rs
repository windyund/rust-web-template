...
use crate::{config::CUSTOM_AUTHORIZATION_ERROR, db::axredis::get_redis_client, utils::jwt};


/**
 * @desc 自定义请求头验证
 */
pub async fn verify_auth(request: Request, next: Next) -> impl IntoResponse {
    let auth = env::var("AUTH").unwrap_or("1".to_string());
    if auth == "0" {
    ...
    if request.headers().contains_key("Authorization") {
        ...
        let login_token_string: String = match redis::cmd("GET").arg(user_token_key).query(&mut con)
        {
            Ok(login_token) => {
